...
namespace RootMotion.FinalIK {
	...
	public class InteractionObject : MonoBehaviour {
		...
		{
			[HideInInspector]
			public InteractionObject componentHoldingThisMessage;
			...
			{
				//Health Scholars edit - added error handling
				if (recipient == null || string.IsNullOrWhiteSpace(function))
                {
					Debug.LogError($"One of the references on '{componentHoldingThisMessage?.gameObject.name}' messages is missing.", componentHoldingThisMessage);
					return;
                }
		...
		void SetParentReference()
        {
			foreach (var interactionEvent in events)
            {
                ...
                {
					message.componentHoldingThisMessage = this;
                }
            }
        }
